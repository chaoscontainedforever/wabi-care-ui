...
import { Search, BookOpen, Target, CheckCircle2 } from "lucide-react"
...
interface GoalBankModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSelectGoal: (goal: Goal) => void
  mode: "select" | "save" // "select" for choosing from bank, "save" for saving to bank
  currentGoal?: Partial<Goal> // Current goal data when saving
  onSave?: (goal: Partial<Goal>) => void
}

// Mock goal bank data - in production, this would come from API/database
...
export function GoalBankModal({ 
  ...
  const [saveTitle, setSaveTitle] = useState(currentGoal?.title || "")
  const [saveCategory, setSaveCategory] = useState(currentGoal?.category || "")
  ...
  const handleSaveToBank = () => {
    if (!currentGoal || !saveTitle.trim()) {
      alert("Please provide a goal title")
      return
    }

    const goalToSave: Partial<Goal> = {
      ...currentGoal,
      title: saveTitle,
      category: saveCategory || currentGoal.category || "General",
      createdAt: new Date().toISOString()
    }
    ...
    console.log("Saving goal to bank:", goalToSave)
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Save to Goal Bank</DialogTitle>
            <DialogDescription>
              Save this goal to the Goal Bank for future use
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., Social, Academic, Behavioral"
              />
            </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a goal from the Goal Bank to add to your treatment plan
          </DialogDescription>
        </DialogHeader>
